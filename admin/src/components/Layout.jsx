...
export default function Layout() {
    ...
    const [siteName, setSiteName] = useState('NewsReact')

    useEffect(() => {
        const fetchSettings = async () => {
            try {
                const { data } = await api.get('/settings')
                // Public /settings endpoint returns object directly with logo key
                if (data.logo) {
                    setLogo(data.logo)
                }
                if (data.site_name) {
        ...
        <div className="admin-layout">
            <aside className="sidebar">
                <div className="sidebar-logo">
                    {logo ? (
                        <img src={logo} alt="Logo" style={{ height: '2rem', width: 'auto' }} />
                    ) : (
                        <span>{siteName}</span>
                    )}
                </div>
                ...
                <nav className="sidebar-nav">
                    ...
                    <NavLink to="/media" className={({ isActive }) => `sidebar-link ${isActive ? 'active' : ''}`}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    {isAdmin && (
                        <div className="sidebar-section">
                            ...
                            <NavLink to="/pages" className={({ isActive }) => `sidebar-link ${isActive ? 'active' : ''}`}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
                                    <polyline points="14 2 14 8 20 8"></polyline>
                                </svg>
                ...
                <div style={{ marginTop: 'auto', paddingTop: '2rem' }}>
                    ...
                    <button onClick={logout} className="sidebar-link" style={{ width: '100%', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left' }}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
